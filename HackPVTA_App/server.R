...
current.time <- as.numeric(PVTA.God.Mode.38.Data$Time[1])
...
shinyServer( function(input, output) {
  ...
  observeEvent(input$btn.back.day, {
    ...
    output$map <- renderPlot({
      current.busses <- filter(PVTA.God.Mode.38.Data, Time > current.time - 100 & Time < current.time + 100)
      PlotOnStaticMap(PVTA.God.Mode.Map, current.busses$Latitude, current.busses$Longitude, col='red', type="p")
    })
  })
